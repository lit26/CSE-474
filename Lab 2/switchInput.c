...
void PortF_init(void){
  ...
  GPIOF_IS_R &= ~0x11; // make bit 4, 0 edge sensitive
  GPIOF_IBE_R &= ~0x11; // trigger is controlled by IEV
  GPIOF_IEV_R  = ~0x11; // falling edge trigger
  GPIOF_ICR_R |= 0x11; // clear any prior interrupt
  GPIOF_IM_R  |= 0x11; // unmask interrupt
...
void PortF_Handler (void){
  if ((GPIOF_DATA_R & 0x01) == 0x00){//SW1
    color = 2;  
    T0_CTL = 0x00;      
  }else if ((GPIOF_DATA_R & 0x10) == 0x00){//SW0
    T0_ICR = 0x01;
    T0_CTL |= 0x01; 
  }
  GPIOF_ICR_R |= 0x11; //clear flag
}
